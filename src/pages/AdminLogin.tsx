...
const AdminLogin = () => {
    ...
    try {
      ...
      if (success) {
        toast({
          ...
          description: "مرحباً بك في لوحة تحكم شبكة الحاج نت",
        });
        navigate("/admin");
      } else {
        toast({
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-sky-50 to-blue-100 bg-wifi-pattern p-4">
      <Card className="w-full max-w-md shadow-xl border-wifi-accent/30">
        <CardHeader className="text-center">
          <div className="mx-auto bg-blue-100 p-3 rounded-full w-16 h-16 flex items-center justify-center mb-2">
            <ShieldAlert className="h-8 w-8 text-wifi-primary" />
          </div>
          <CardTitle className="text-2xl">تسجيل دخول المسؤول</CardTitle>
          <CardDescription>أدخل بيانات المسؤول للوصول إلى لوحة التحكم</CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="username">اسم المسؤول</Label>
              <Input
                id="username"
                type="text"
            ...
            <div className="text-sm text-muted-foreground mt-2">
              <p>للتجربة استخدم: admin / admin123</p>
            </div>
          </CardContent>
          <CardFooter className="flex flex-col gap-4">
            <Button type="submit" className="w-full" disabled={isLoading}>
              {isLoading ? "جاري تسجيل الدخول..." : "تسجيل دخول المسؤول"}
            </Button>
